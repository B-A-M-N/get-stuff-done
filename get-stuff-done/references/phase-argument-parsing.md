...
```bash
PHASE_INFO=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
