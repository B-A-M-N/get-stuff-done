...
```bash
INIT=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" init milestone-op)
...
```bash
integration_checker_model=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" resolve-model gsd-integration-checker --raw)
```
...
node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```
