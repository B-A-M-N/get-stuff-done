...
PHASES=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" phases list)
...
```bash
node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
