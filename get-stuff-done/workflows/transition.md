...
```bash
TRANSITION=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" progress bar --raw)
```
