...
```bash
# Claude Code
npx get-stuff-done-cc --claude --global   # 安装到 ~/.claude/
npx get-stuff-done-cc --claude --local    # 安装到 ./.claude/

# OpenCode（开源，可用免费模型）
npx get-stuff-done-cc --opencode --global # 安装到 ~/.config/opencode/

# Gemini CLI
npx get-stuff-done-cc --gemini --global   # 安装到 ~/.gemini/

# Codex（以 skills 为主）
npx get-stuff-done-cc --codex --global    # 安装到 ~/.codex/
npx get-stuff-done-cc --codex --local     # 安装到 ./.codex/

# 所有运行时
npx get-stuff-done-cc --all --global      # 安装到所有目录
```
...
**命令行为不符合预期？**
- 运行 `/gsd:help` 确认安装成功
- 重新执行 `npx get-stuff-done-cc` 进行重装
...
OpenCode、Gemini CLI 和 Codex 现在都已经通过 `npx get-stuff-done-cc` 获得原生支持。
