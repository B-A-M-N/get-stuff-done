...
```bash
node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" history-digest
```
...
```bash
VALID=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "$HOME/.claude/get-stuff-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
